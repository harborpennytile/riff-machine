...
function extractItems(t){const items=[];const re=/\{[^{}]*"type"\s*:\s*"[^"]+?"[^{}]*"title"\s*:\s*"[^"]*?"[^{}]*\}/g;let m;while((m=re.exec(t))!==null){try{const o=JSON.parse(m[0]);if(o.type&&o.title)items.push(o)}catch{}}return items}
