const ICONS = { article: "Article", visual: "Visual", music: "Music", book: "Book", concept: "Concept", person: "Person" };
...
export function exportSeedMarkdown(seed) {
  ...
  for (const [type, items] of Object.entries(byType)) {
    const typeIcon = ICONS[type] || "Other";
    md += `## ${typeIcon}\n\n`;
    ...
    for (const syn of syntheses) {
      ...
      if (syn.leads?.length) {
        ...
        for (const lead of syn.leads) {
          const leadSearchUrl = lead.search
            ? `https://www.google.com/search?q=${encodeURIComponent(lead.search)}`
            : `https://www.google.com/search?q=${encodeURIComponent(lead.title)}`;
          md += `- **[${lead.title}](${leadSearchUrl})** -- ${lead.desc || lead.description || ""}\n`;
        }
        md += `\n`;
      }
      md += `---\n\n`;
